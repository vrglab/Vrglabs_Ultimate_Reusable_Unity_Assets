...
public class Slot<t> where t : ItemType
{
    ...
    public bool CanBePlaced(t item)
    {
        return stackHandler.CanBePushed(item);
    }
    ...
    public void Push(t item, params object[] args)
    {
        if (stackHandler.currentStackSize < stackHandler.maxSize)
        {
            stackHandler.Put(item, args);
            ActiveItemType = item.GetType();
            Full = true;
        }
    }
